...
test("검색·분류·모바일 목차가 실제 문서 메타데이터를 사용한다", async () => {
  ...
  assert.match(homeHtml, new RegExp(`<strong>${result.wiki.stats.statuses.active ?? 0}</strong><small>활성</small>`));
  assert.match(homeHtml, new RegExp(`<strong>${result.wiki.stats.statuses.draft ?? 0}</strong><small>초안</small>`));
  assert.match(homeHtml, new RegExp(`<strong>${result.wiki.stats.statuses.review ?? 0}</strong><small>검토</small>`));
...
test("Brutalist 법률 공보형 디자인 토큰을 일관되게 사용한다", async () => {
  ...
  assert.doesNotMatch(css, /@font-face|D2Coding/i);
...
test("시스템 글꼴과 정적 자산 예산을 지킨다", async () => {
  const css = await fs.readFile(path.join(rootDir, "site", "assets", "styles.css"), "utf8");
  assert.doesNotMatch(css, /@font-face|D2Coding/i);
  assert.equal(await fs.stat(path.join(outputDir, "assets", "fonts")).catch(() => null), null);
