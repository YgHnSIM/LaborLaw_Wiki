...
- 분석 문서에서 특정 문단이나 쟁점의 근거 범위를 표시할 때는 문장 끝에 `[@SRC-UNIQUE-ID]` 형식의 근거 표식을 사용할 수 있습니다. 표식의 ID는 같은 페이지의 `source_refs`에 등록되어 있어야 하며, 웹 빌드에서는 근거 패널의 해당 출처 번호로 변환됩니다.
- 근거 표식은 주장과 출처의 대응이 확인된 경우에만 사용합니다. 페이지 전체의 포괄적 참고자료를 문단 근거처럼 표시하거나, 하나의 출처가 뒷받침하지 않는 복수 주장을 한 표식에 묶지 않습니다.
...
- 본문에는 `wiki/sources/`의 소스 요약 페이지로 향하는 위키링크를 붙이지 않습니다. 문단·쟁점 단위 대응이 필요하면 등록된 `[@SRC-ID]` 근거 표식을 사용합니다.
